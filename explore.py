...
class MiRoClient:
    ...
    def detect_carer(self, frame, index):
        if frame is None:  # Sanity check
            return
        ...
        # Specify target ball colour
        rgb_colour = np.uint8([[[0, 0, 255]]])  # e.g. red (Note: BGR)
        # Convert this colour to HSV colour model
        hsv_colour = cv2.cvtColor(rgb_colour, cv2.COLOR_RGB2HSV)
        ...
        # Get the hue value from the numpy array containing target colour
        target_hue = hsv_colour[0, 0][0]
        hsv_lo_end = np.array([70, 70, target_hue - 20])
        hsv_hi_end = np.array([255, 255, target_hue + 20])
    ...
    def look_for_carer(self):
        for index in range(2):  # For each camera (0 = left, 1 = right)
            ...
            image = self.input_camera[index]
            # Run the detect ball procedure
            self.ball[index] = self.detect_carer(image, index)
        if not self.ball[0] and not self.ball[1]:
            self.drive(self.FAST, self.SLOW)
        else:
            self.carer_found = True

    def turn_to_carer(self):
        self.reset_head_pose()
        if not self.carer_found:
            self.look_for_carer()
        #turning:
        else:
            for index in range(2):  # For each camera (0 = left, 1 = right)
    ...
    def loop(self):
        ...
        self.status_code = 0
        self.carer_found = False
        self.modes = Enum('mode', ['PLAY', 'APPROACH'])
        self.mode = self.modes.APPROACH
        self.need_for_care = 1
        while not rospy.core.is_shutdown():

            if self.mode == self.modes.PLAY:
                if self.counter == 1:
                    print ("Miro in Play mode")
                self.play()
                if self.counter > 100: #need for care increases past threshold
                    self.need_for_care += 1
            elif self.mode == self.modes.APPROACH:
                if self.counter == 1:
                    print ("Miro in Approach mode")
                    self.carer_found = False
                self.approach()
                if self.counter > 10: #satisfied need for care
                    self.need_for_care -= 1
            rospy.sleep(self.TICK)

            if self.need_for_care == 0 and self.mode != self.modes.PLAY:
                print ("switch")
                self.mode = self.modes.PLAY
                self.counter = 0
                self.status_code = 0
            elif self.need_for_care == 1 and self.mode != self.modes.APPROACH:
                print ("switch")
                self.mode = self.modes.APPROACH
                self.counter = 0
            print (self.counter)
