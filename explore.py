    ...
    def loop(self):
        ...
        while not rospy.core.is_shutdown():
            ...
            if self.mode == self.modes.PLAY:
                ...
                self.play()
                if self.counter > 1000: #need for care increases past threshold
                    self.need_for_care += 1
            elif self.mode == self.modes.APPROACH:
                if self.counter == 1:
                    print ("Miro in Approach mode")
                self.counter += 1 #approach behaviour
                if self.counter > 1000: #satisfied need for care
                    self.approach()
                    self.need_for_care -= 1
            rospy.sleep(self.TICK)
